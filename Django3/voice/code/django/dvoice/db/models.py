...
from dvoice import dsh_django_utils,dsh_django_config,dsh_django_utils2
from dvoice import dsh_db_config
...
class Person(DshObject):
    ...
    def meta_display(self):
        return self.person_item_link() + \
               ...
               self.pin_dsh_uid() + \
               self.session_link() + \
               dsh_common_db.models_person_heard_messages_icon(self) +\
               self.dial_now_link() + \
               self.selected_icon()
    meta_display.short_description = 'attrs.'
    meta_display.allow_tags = True
    ...
    def session_link(self):
        spaces = dsh_django_config.lookup('ICON_SPACES')
        sessionStr = dsh_django_utils.session_link(self.session)
        if sessionStr:
            return sessionStr + spaces
        return ''
...
class Item(DshObject):
    ...
    def testdv1(self):
        dsh_django_utils.debug_event('item.save.testdv1(): entered!', 33)
    ...
    def save(self, recurse=False, noLogging=False, sessionID='',
        ...
        dsh_django_utils.debug_event('item.save: test that', 33)
    ...
    def email_text(self, br=False, attach=False, urlFields=None,
        ...
        if self.description:
            ...
            if br:
                text += u'<BR>'
        if urlFields and urlFields['dsh_url']:
            #
            # 10/03/06:
            ...
            url = 'URL: <a href="%s">%s</a>' % (fullURL, fullURL)
            if allowTags:
                text += url + '\n'
            else:
                text += 'URL: ' + fullURL + '\n'
