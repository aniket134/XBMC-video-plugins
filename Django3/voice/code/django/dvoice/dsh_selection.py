...
def keyword_add_del(dshUid, action='add'):
    ...
    errorMsg = ''
    keyWord = dsh_dump.get_foreign_key(dvoice.db.models.KeyWord, dshUid)
    if not keyWord:
    ...
    selectedItems = dvoice.db.models.Item.objects.filter(u17=True)
...
def select_starred():
    ...
    starred = dvoice.db.models.Item.objects.filter(starred=True)
    if not starred:
        response += dsh_utils.red_error_break_msg(
            'no starred item.')
        return response
...
def process_selection(fieldStr, action='set'):
    """
    called by views.selection_deactivate(request), etc.
