...
def unknown_thumbnail(obj, noCenter=False):
    if isinstance(obj, dv2.db.models.Person):
        if noCenter:
            return dsh_django_config.lookup(
                'DSH_UNKNOWN_HEAD_URL_NOT_CENTERED')
        return dsh_django_config.lookup('DSH_UNKNOWN_HEAD_URL')
    elif isinstance(obj, dv2.db.models.Organization):
...
def thumbnail(self, picField, noCenter=False):
    ...
    if not picField:
        return unknown_thumbnail(self, noCenter=noCenter)
    ...
    if noCenter:
        return ('<img src="%s" width="%s" height="%s" title="%s" />' +
            '') % (url, thumbWidth, thumbHeight, '')
...
def insert_event_after_upload_item(self, sessionID=''):
    event = dv2.db.models.Event(etype='INF',
                                   action='UPLD',
                                   owner=self.owner,
...
def debug_event(message, tag, sessionID=''):
    ...
    if tag != currentPrintTag:
        return
    event = dv2.db.models.Event(etype = 'DBG',
                                   action = 'PRNT',
                                   debug_tag = tag,
                                   session=sessionID)
...
def error_event(message, errorLevel='ERR', item=None, phone_number='',
    ...
    if item:
        ...
        dsh_uid = ''
    event = dv2.db.models.Event(etype = errorLevel,
                                   action = 'RPRT',
...
def deactivate(item, recurse=False):
    ...
    dsh_common_db.deactivate(item, dv2.db.models.Item, recurse=recurse)
...
def convert_field_to_sln(field, item=None):
    ...
    debug_event('dsh_django_utils.convert_to_sln: entered...', dbt)
    ...
    debug_event('dsh_django_utils.convert_to_sln: chmod: ' + mp3FilePath, dbt)
    ...
    if item:
        ...
        if item.itype != 'B' and item.itype != 'P' and (not item.peer_shared):
            #
            # 10/03/06:
            # I'm now converting everything.
            # this is done when I decided to play back the
            # questions before saying answers.
            # so questions should be converted too.
            # see dsh_common_agi.check_personalized_followup_to()
            # see getting chopped in there.
            #return
            pass
    ...
    if not is_mp3(url):
        return
    debug_event('dsh_django_utils.convert_to_sln: going ahead: ' + url, dbt)
...
def get_original_message_owner(originalItemDshUid):
    ...
    response = ''
    item = get_foreign_key(dv2.db.models.Item, originalItemDshUid)
    if not item:
...
def save_red5_mp3_in_django(mp3Path, originalItemDshUid=None,
    ...
    #
    # 10/03/26:
    # if copyBlankReply, then we're getting called by
    # dsh_common_views.reply_upload_submit().
    # let's add an "answers" key.
    #
    if copyBlankReply:
        dsh_common_db.add_key_word_to_item(
            item, 'answers', "doctor's answers",
            dv2.db.models.Item, dv2.db.models.KeyWord)
...
def check_auto_timed_calls_for_all_persons(noLogging=False):
    ...
    persons = dv2.db.models.Person.objects.filter(timed1__isnull=False)
...
def should_auto_dial_broadcast(callee):
    ...
    10/02/24: for Lokarpit.
    make it less reluctant to put out the broadcast message.
    I mean previously, for an auto-dial or for a call-now,
    if the person is signed for auto-dial slots, he won't hear the broadcast.
    now let's turn this off.
    """

    funcName = 'dsh_django_utils.should_auto_dial_broadcast:'
    
    #
    # 10/04/14:
    # no broadcast for doctors.
    #
    if callee.ptype == 'DOC':
        dsh_utils.db_print(
            '%s no broadcast for doctors.' % (funcName,), 165)
        return None
    ...
    #actives = dv2.db.models.Item.objects.filter(active=True, itype='B')
    actives = dsh_common_db.get_active_broadcast_item_for_caller(
        callee, dv2.db.models.Item, dv2.db.models.KeyWord, None)
    if not actives:
        return None
    ...
    for active in actives:
        heardEvents = dv2.db.models.Event.objects.filter(
...
def check_auto_timed_calls_for_person(callee, noLogging=False,
    ...
    #
    # 10/03/31:
    # am not looking at the callee.timed_broadcast flag because...
    # originally, the intention was to auto-dial Calcutta and
    # they may not need to hear broadcasts meant for people here.
    # this flag also got commented above in should_auto_dial_broadcast()
    # during LV trips because we wanted more liberal broadcast policies.
    #


    #
    # 10/03/31:
    # see dsh_common_agi.py, note100331.
    # if we get here, there's nothing to play, but because the person is
    # in the CDS, we'll give this guy a call once.  it'll get turned off
    # as soon as this guy calls just once.
    #
    if callee.current_dial:
        success,scheduled = auto_schedule_one_callee(
            None, callee, force=True, noLogging=noLogging,
            sessionID=sessionID)
        debug_event('dsh_django_utils.check_auto_timed_calls_for_person: '+\
                    'forced call for CDS: ' + callee.__unicode__(), 42)
        return (scheduled, 'attempted to schedule for CDS.')


    #
    # 10/03/31:
    # this has an issue with respect to playing peer-shared to
    # doctors.  right now there's no peer-shared so we don't worry
    # about it.
    #
    ...
    for msg in messages:
        ...
        # a fix.
        #
        eventTable = dv2.db.models.Event
...
def auto_schedule_one_callee(item, callee, force=False, noLogging=False,
    ...
    debug_event('dsh_django_utils.auto_schedule_one_callee: entered: ' +\
                repr(item) + ',  noLogging: ' + repr(noLogging), 30)
    
    #scheduledEvents = dv2.db.models.Event.objects.filter(
    #    action='SCHE', owner=callee.id, dsh_uid_concerned=item.dsh_uid)
    #if scheduledEvents:
    #    return (True, False)

    if not force and item:
        heardEvents = dv2.db.models.Event.objects.filter(
...
def schedule_next_call_time(callee):
    ...
    for checkThis in checkList:
        ...
        debug_event('dsh_django_utils.schedule_next_call_time(): ' +\
                    ...
                    'scheduled: ' + repr(scheduledTime), 40)
        if scheduledTime < resultTime:
            resultTime = scheduledTime

    debug_event('dsh_django_utils.schedule_next_call_time(): ' +\
                repr(resultScheduled) + ' -- ' + repr(resultTime), 40)
...
def make_dot_call_file(fileContent, callTime, item, phoneNum, callee,
    ...
    try:
        ...
        # dialNow=True, but I did set callTime to be 5 seconds in the future.
        #
        if (not dialNow) or callTime:
            op = 'time.mktime'
...
def display_phone_link(person):
    ...
    persons = dv2.db.models.Person.objects.filter(phone_number=phoneNumber)
    if not persons:
        return hiddenNumber
    howMany = len(persons)
...
def auto_schedule_list():
    ...
    for one in listing:
        ...
        # person thumbnail.
        #
        dshUidLen = dv2.db.models.dsh_uid_length()
        dshUid = one[:dshUidLen]
        person = get_foreign_key(dv2.db.models.Person, dshUid)
        ...
        if not person:
            ...
            personID = str(person.id)
            personalMsgs = dv2.db.models.Item.objects.filter(
        ...
        if not person:
            ...
            if not person.timed_broadcast:
                ...
                #broadcastMsgs = dv2.db.models.Item.objects.filter(
                #    active=True, itype='B')
                broadcastMsgs = \
...
def unknown_list():
    ...
    unknowns = dv2.db.models.Person.objects.filter(first_name='no-name')
    if not unknowns:
...
def phone_number_list():
    ...
    people = dv2.db.models.Person.objects.all()
...
def get_by_dsh_uid(dshUid):
    ...
    types = [
        [dv2.db.models.Item, 'item'],
        [dv2.db.models.Person,'person'],
        [dv2.db.models.Organization, 'organization'],
        [dv2.db.models.KeyWord, 'keyword'],
        [dv2.db.models.Event, 'event'],
    ]
...
def check_spoken_names(kind='peer_shared'):
    ...
    if kind == 'peer_shared':
        itemList = dv2.db.models.Item.objects.filter(peer_shared=True)
        if not itemList:
        ...
        itemList = dv2.db.models.Item.objects.filter(
            active=True, itype='P')
        if not itemList:
...
def dial_now(dshUid):
    ...
    callee = get_foreign_key(dv2.db.models.Person, dshUid, quiet=True)
    if not callee:
        response = dsh_utils.red_error_break_msg(
...
def people_heard_link(item):
    ...
    if not heardEvents:
        unheardEvents = dv2.db.models.Event.objects.filter(
            action='UNHR',
            dsh_uid_concerned=item.dsh_uid)
...
def session_includes_saved(person, sessionID):
    ...
    if not sessionID:
        ...
        saveEvents = dv2.db.models.Event.objects.filter(
            etype='INF',
            action='UPLD',
            owner=person,
...
"""
    ...
    item = get_foreign_key(dv2.db.models.Item, dshUid)
    if not item:
        return emptyResponse
