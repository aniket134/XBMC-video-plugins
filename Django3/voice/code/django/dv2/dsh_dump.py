...
allDbTables = {
    'keyword': [dv2.db.models.KeyWord, 'key word(s)'],
    'organization': [dv2.db.models.Organization, 'organization(s)'],
    'person': [dv2.db.models.Person,'person(s)'],
    'item': [dv2.db.models.Item, 'item(s)'],
    'event': [dv2.db.models.Event, 'event(s)'],
}
...
def load_one(dump, whatKind, overWrite, itemsToSkip=None):
    ...
    for fieldName,fieldContent in dump.iteritems():
        ...
        if fieldType == 'RequiredForeignOrgType':
            ...
            #
            org = get_foreign_key(dv2.db.models.Organization, fieldValue)
            if not org:
                dsh_utils.give_bad_news(
        ...
        if fieldType == 'RequiredForeignPersonType':
            ...
            #
            person = get_foreign_key(dv2.db.models.Person, fieldValue)
            if not person:
                dsh_utils.give_bad_news(
                    'dsh_dump.load_one: ' +\
...
def load_pointers_for_one(dump, whatKind, overWrite, itemsToSkip):
    ...
    for fieldName,fieldContent in dump.iteritems():
        ...
        if fieldType == 'OptionalKeyWordsType':
            ...
            #
            keyWords = get_foreign_key_list(dv2.db.models.KeyWord,
                                            fieldValue)
            if keyWords == None:
                #
        ...
        if fieldType == 'OptionalPersonsType':
            ...
            #
            audience = get_foreign_key_list(dv2.db.models.Person,
                                            fieldValue)
            if audience == None:
                #
                # this shouldn't happen. in the worst case, it should've
...
def load_all(allKeyWords, allOrgs, allPersons, allItems, allEvents, overWrite):
    ...
    for keyWord in allKeyWords:
        load_one(keyWord, dv2.db.models.KeyWord, overWrite)
    ...
    dsh_utils.give_news('dsh_dump.load_all: loading organizations...',
                        logging.info)
    for org in allOrgs:
        load_one(org, dv2.db.models.Organization, overWrite)
    ...
    for person in allPersons:
        load_one(person, dv2.db.models.Person, overWrite)
    ...
    for item in allItems:
        load_one(item, dv2.db.models.Item, overWrite,
                 itemsToSkip=itemsToSkip)
    ...
    dsh_utils.give_news('dsh_dump.load_all: loading pointers within items...',
                        logging.info)
    for item in allItems:
        load_pointers_for_one(item, dv2.db.models.Item, overWrite,
                              itemsToSkip)
...
def propagate_selections():
    ...
    # need to do more.
    #
    mark_selected_foreign(dv2.db.models.Item, 'owner')
    mark_selected_foreign(dv2.db.models.Item, 'intended_audience',
                          kind='many')
    mark_selected_foreign(dv2.db.models.Item, 'i05')
    dsh_django_utils.debug_event(
        'propagate_selections: mark event owner...', 11)
    mark_selected_foreign(dv2.db.models.Event, 'owner')
    mark_selected_foreign(dv2.db.models.Person, 'organization')
    mark_selected_foreign(dv2.db.models.Item, 'key_words', kind='many')
