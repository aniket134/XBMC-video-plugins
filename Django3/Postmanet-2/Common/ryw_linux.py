...
def write_script_prefix(f, g, modpaths, baseName):
    ...
    if baseName in ryw.RYW_ISO_ENCODING_PREFIX_FILES:
        g.write(ryw.RYW_ISO_ENCODING_PREFIX + "\n")
...
def move_stuff(src, dst):
    ...
    shutil.move(src, dst)
    ryw.db_print2('ryw_linux:move_stuff: src is: ' + src +
                  ' --->  dst is: ' + dst, 56)
...
def launch_explorer(path):
    ...
    if os.path.exists('/usr/local/bin/rox'):
        ryw.db_print_info_browser('launch_explorer: found rox.', 91)
        command = '/usr/local/bin/rox '
    elif os.path.exists('/usr/bin/thunar'):
        ryw.db_print_info_browser('launch_explorer: found thunar.', 91)
        command = '/usr/bin/thunar --display :0.0 '
    else:
        ryw.give_bad_news("launch_explorer: can't find an explorer.",
                          logging.error)
        return False
        
    command = command + path + ' &'
    ryw.db_print_info_browser('launch_explorer: command is: ' + command, 91)
...
def stdio_logs_open(logDirectory=''):
    ...
    try:
        ...
        ryw.give_bad_news('stdio_logs_open: failed to open log files.',
                          logging.error)
        return None
...
def sudo_chmod(path, smode='777', imode=0777, repositoryRoot = None):
    ...
    if not ryw.is_valid_file(chmodPath):
        ryw.give_bad_news('Cannot find suid chmod. Advise you to fix this.',
                          logging.error)
        return
...
def dos2unix(fileName):
    ...
    if not os.path.exists(commandPath):
        ryw.give_bad_news("ryw_linux.dos2unix: dos2unix doesn't exist. " +
    ...
    if ret != 0:
        ryw.give_bad_news('ryw_linux.dos2unix: error in running dos2unix: ' +
                          fileName, logging.error)
        return False
    else:
        ryw.db_print_info_browser('ryw_linux.dos2unix: dos2unix success: ' +
                                  fileName, 69)
...
def run_dos2unix_reread(fileName):
    ...
    value = ryw.pickle_load_raw_and_text(fileName, dos2unix=True)
    return value
