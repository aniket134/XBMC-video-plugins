...
def free_MB(path):
    ...
    #tested.
    ryw.db_print('ryw_xp:free_MB() entered...', 49)
    
    path = os.path.normpath(path)
    if not ryw.try_mkdir_ifdoesnt_exist(path, 'free_MB'):
        ryw.give_bad_news(
...
def flock_init(flockSelf, file):
    ...
    #tested.
    ryw.db_print('ryw_xp:flock_init() entered...', 49)
    fl = flockSelf
...
def flock_unlock(flockSelf):
    ...
    #tested
    ryw.db_print('ryw_xp:flock_unlock() entered...', 49)
    fl = flockSelf
